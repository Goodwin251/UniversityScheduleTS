import {Professor, Classroom, Course, Lesson, ScheduleConflict, DayOfWeek, TimeSlot, CourseType} from "./types.js"
...
validateLesson(lesson: Lesson): ScheduleConflict | null {
    ...
    const professorConflict = this.schedule.find(l => 
    ...
    const classroomConflict = this.schedule.find(l => 
    ...
    const courseIdConflict = this.schedule.find(l =>
...
reassignClassroom(lessonId: number, newClassroomNumber: string): boolean {
    ...
    if (lesson && !this.schedule.find(l => l.classroomNumber === newClassroomNumber && l.timeSlot === lesson.timeSlot && l.dayOfWeek === lesson.dayOfWeek)) {
...
getMostPopularCourseType(): CourseType {
    ...
    }, { type: "Lecture", count: 0 }).type as CourseType;
}    
}